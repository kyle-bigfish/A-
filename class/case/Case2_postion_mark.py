...
# ===导入数据
df = pd.read_hdf('D:\python\pycharm\\untitled\\homework\data\signals.h5', key='df')


# ===由signal计算出实际的每天持有仓位
# 在产生signal的k线结束的时候，进行买入,当天2：55分买入，卖出也是2.55分
df['signal'].fillna(method='ffill', inplace=True)#使用之前的值进行补全
...
# ===将数据存入hdf文件中
df.to_hdf('D:\python\pycharm\\untitled\\homework\data\pos.h5', key='df', mode='w')
